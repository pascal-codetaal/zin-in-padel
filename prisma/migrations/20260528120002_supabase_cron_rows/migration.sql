...
--
-- Prisma shadow DB: skipped (no pg_cron / Vault on shadow databases).
--
...
BEGIN
  IF current_database() LIKE 'prisma_migrate_shadow_db%' THEN
    RAISE NOTICE 'supabase_cron_rows: skipped on Prisma shadow DB %', current_database();
    RETURN;
  END IF;

  FOR r IN
    SELECT jobid FROM cron.job WHERE jobname IN ('cascade-tick', 'send-tick')
  LOOP
